...
def pixel_accuracy(prediction:torch.Tensor, target:torch.Tensor):
    ...
    return (out == target.float()).float().mean().item()
...
if __name__=='__main__':
    ...
    tu.load_model_checkpoint(model, 'C:/Users/arind/Downloads/model.torch', inference=False, map_location=device)
    model.to(device)
    print("Model loaded")
    ...
    # build data loader
    tu_test_dataloader = DataLoader(tu_test_dataset, batch_size=cc.test_batch, shuffle=True, num_workers=1)
